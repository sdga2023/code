...
<CategoricalLegend
  legendType={'custom'}
  customItems={[
    { key: energy, colour: Colors.ColorBaseYellow200 },
    { key: nonenergy, colour: Colors.ColorBaseTeal300 }
  ]}
/>
  ...
  <g transform={'translate(' + margins.left + ',' + margins.top + ')'}>
    {#each treemapData.leaves() as box}
      <rect
        ...
        stroke="white"
        stroke-width={1}
        opacity={box.data.sector == 'Energy' && activeScene.id == 'energy'
          ? 1
          : box.data.sector == 'Non-energy' && activeScene.id == 'nonenergy'
          ? 1
          : 0.1}
    ...
    {#each treemapData.descendants().filter((d) => d.depth == 1) as label}
      <text
        ...
        x={label.x0 + 4}
        y={label.y0 + 18}
        opacity={label.data[0] == 'Energy' && activeScene.id == 'energy'
          ? 1
          : label.data[0] == 'Non-energy' && activeScene.id == 'nonenergy'
          ? 1
    ...
    {#each treemapData.descendants().filter((d) => d.depth == 2) as label}
      <text
        ...
        text-anchor={'middle'}
        opacity={label.data[1][0].sector == 'Energy' && activeScene.id == 'energy'
          ? 1
          : label.data[1][0].sector == 'Non-energy' && activeScene.id == 'nonenergy'
          ? 1
...
{#if hoverData && mousePos}
  <Tooltip visible={true} targetPos={mousePos}>
    <p>
      ...
      <span class="label caps stronger">({energyMap[hoverData.data.sector]})</span>
    </p>
    <p class="label">
